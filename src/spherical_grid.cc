...
double SphericalGrid::smallest_wall_size(Ray *R) {
    ...
    // Calculate the minimum size in the theta direction, with a minimum theta
    // of 0.05 and maximum of 0.3.
    double st = fabs(r*fmax(fmin(w2[R->l[1]+1] - w2[R->l[1]],0.3),0.05));
    
    // Calculate the minimum size in the phi direction.
    double sint = sin(0.5*(w2[R->l[1]] + w2[R->l[1]+1]));

    double sp = fabs(r * sint * fmax(fmin(w3[R->l[2]+1] - w3[R->l[2]],0.3),
            0.05));

    // Try to account for projection effects, a la RADMC3D.
    double proj = fabs(R->n[0]*R->r[0] + R->n[1]*R->r[1] + R->n[2]*R->r[2]) / 
        R->rad;
    double sproj = (fabs(R->n[2])*st + (1.0 - fabs(R->n[2]))*sp) * proj;

    double s = fmax(sr, fmin(st, fmin(sp, sproj)));
